...
Raw PDF documents are uploaded, linked to corpora, ingested, chunked, and stored for retrieval. The system supports both corrective RAG and GraphRAG flows and records sources in an evidence ledger. Recent security changes also add stricter PDF upload validation, and recent prompt work now intercepts simulation turns and learner questions at the service layer before graph or agent invocation, with guard checks also enforced in the shared LLM invocation path.
