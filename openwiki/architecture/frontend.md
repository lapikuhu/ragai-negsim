...
- `UserDetailPage.tsx` is the admin-facing, role-aware user activity view. Students show simulation participation; teachers add evaluations and authored scenarios, corpora, and personas; admins also show authored RAG profiles and users they created. The expanded detail page now reflects the backend's user-specific attribution data directly in the UI.
...
- `KnowledgeGraphsPage.tsx`, `FullCorpusIndexPipeJobsPage.tsx`, and `VectorStoresPage.tsx` support the retrieval infrastructure. The full-corpus form requires a unique chunk-set name, defaults **Build BM25 index also** on, requires a separate user-supplied BM25 name, and permits an explicit dense-only opt-out. The page reflects the persisted `CorpusChunkSet` contract by showing the created set ID, linked BM25 child status, and the BM25-building and artifact-rollback stages.
