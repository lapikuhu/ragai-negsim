...
from app.services.rag_eval_export import (
...
async def export_rag_eval_run_summary_json_srvc(
    run_id: int,
    session: AsyncSession,
) -> bytes:
    """
    Return a completed persisted RAG evaluation run as summary JSON bytes.

    Args:
        run_id (int): The ID of the RAG evaluation run.
        session (AsyncSession): The database session.
    Returns:
        bytes: The JSON representation of the RAG evaluation run summary.
    Raises:
        ValueError: If the RAG evaluation run is not found or not completed.
    """
    run = await rag_eval_repo.get_rag_eval_run_by_id(run_id, session)
    if run is None:
        raise ValueError("RAG evaluation run not found")
    ...
    return serialize_rag_eval_run_summary_json(_run_read(run))
